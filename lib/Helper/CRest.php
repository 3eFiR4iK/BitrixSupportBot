...
class CRest
{
    ...
    protected function callCurl($arParams)
    {
        ...
        //Logger::create('bot')->debug('callCurl', ['data' => [$arParams, $arSettings]]);
        ...
        if($arSettings !== false)
        {
            ...
            try
            {
                ...
                {
                    Logger::create('bot')->debug('error rest', ['data' => [$result, $arParams, $this->domain]]);
    ...
    public function GetNewAuth($arParams)
    {
        $result = [];
        $arSettings = $this->getAppSettings();
        //Logger::create('bot')->debug('new auth', ['data' => [$arSettings, $this->domain]]);
    ...
    private function getAppSettings()
    {
        ...
        //Logger::create('bot')->debug('getAppSettings', ['data' => [$isCurrData, $this->domain]]);
    ...
    protected function getSettingData()
    {
        ...
        $bot = DelsisBotsTable::getList([
            'filter' => ['DOMAIN' => $this->domain],
        ])->fetch();


        if ($bot) {
            if ($bot['CLIENT_ID'] != '' && $bot['CLIENT_SECRET'] != '') {
                $return['C_REST_CLIENT_ID'] = $bot['CLIENT_ID'];
                $return['C_REST_CLIENT_SECRET'] = $bot['CLIENT_SECRET'];
            }

            $return = array_merge(json_decode($bot['APP_SETTINGS'], true) ?? [], $return);
        }
        //Logger::create('bot')->debug('getSettingData', ['data' => [$return, $this->domain]]);
    ...
    {
        $res = DelsisBotsTable::getList(['filter' => ['DOMAIN' => $this->domain]])->fetch();

        if ($res) {
            DelsisBotsTable::update($res['ID'], [
                'APP_SETTINGS' => json_encode($arSettings)
            ]);
     ...
     */
    public function checkServer($print = true)
    {
        $return = [];
        ...
        //creat setting file
        file_put_contents(__DIR__ . '/settings_check.json', $this->wrapData(['test'=>'data']));
